...
class AppState(rx.State):
    ...
    def handle_login(self):
        ...
        try:
            password_match = _bcrypt.checkpw(
                self.login_password.encode(), user["password"].encode()
            )
        except Exception:
            password_match = (user["password"] == self.login_password)
    ...
    def handle_register(self):
        ...
        try:
            import bcrypt as _bcrypt
            hashed = _bcrypt.hashpw(self.register_password.encode(), _bcrypt.gensalt()).decode()
            execute(
                "INSERT INTO users (name, email, password, role) VALUES (%s, %s, %s, 'user')",
                (self.register_name, self.register_email, hashed)
            )
        except Exception as e:
